...
export class UserCardService { 
    ...
    async getUserCard(idUser: string, idCard: string) {
    ...
    async setLikeUserCard(type: string, idUser: string, idCard: string, thumbId?: string) {
        ...
        // Faz a requisição HTTP usando Axios para atualizar o servidor, se necessário
        try {
            const response = await axios.post('/api/user-card/like', {
                idUser,
                idCard,
                like: userCard.like
            });
            console.log("Like atualizado no servidor:", response.data);
        } catch (error) {
            console.error("Erro ao atualizar o like no servidor:", error);
        }

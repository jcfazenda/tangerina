...
export class ImagesService {
        ...
        return new Observable(observer => {
            axios.post(`${this.endpoint.trim()}${endpoint}`, body)
                ...
                .catch(error => {
                    ...
                    observer.error('Erro ao buscar imagens do grupo de projetos');
                });
        });
    }
    ...
    getByThumbnail(id: string): Observable<any> {
        ...
        return new Observable(observer => {
            axios.post(`${this.endpoint.trim()}${endpoint}`, body)
                ...
                .catch(error => {
                    this.handleError(error);
                    observer.error('Erro ao buscar imagem pelo thumbnail');
                });
        });
    }

    getImages(id_thumbnail: string): Observable<any> {
        const body = { id_thumbnail };
        const endpoint = 'http://localhost:3333/images/getByThumbnail';
        ...
        return new Observable(observer => {
            axios.post(`${this.endpoint.trim()}${endpoint}`, body)
                ...
                .catch(error => {
                    this.handleError(error);
                    observer.error('Erro ao buscar imagens');
                });
        });
    } 
            ...
            switchMap(compressedBlob => {
                ...
                return new Observable(observer => {
                    axios.post(endpoint, formData)
                        ...
                        .catch(error => {
                            this.handleError(error);
                            observer.error('Erro ao inserir imagem');
                        });
                });
            }),
    ...
    private handleError(error: any): Observable<never> {
        ...
        return throwError(() => new Error('Erro na solicitação. Por favor, tente novamente mais tarde.'));
    }
}
